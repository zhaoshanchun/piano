...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        //NSString *title = [NSString stringWithFormat:NSLocalizedString(@"最多添加 %d 张图片！", nil), self.maximumImagesCount];
        // You can only send %d photos at a time.
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"每次只能添加%d张照片。", nil), self.maximumImagesCount];
        NSString *messageTemp = self.moreThanMaxNumImageMessage?self.moreThanMaxNumImageMessage:message;
